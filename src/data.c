 ...
 * @param offset - pointer to file offset to degrees
 ...
 * @return error code, ==0 - OK, !=0 - error
 */ 
/*int calculateAngles(long double* start[2],uint64_t* count,long double* Dtheta)
{
...
void convertData1(FILE* fileIn,FILE* fileOut,char separator,long double* start,long double* Dtheta)
{
	...
	for(uint64_t ii=0;!feof(fileIn);ii++)
	{
		...
		fprintf(fileOut,"%0.8Lf%c",start[0]+((long double)ii)*(*Dtheta),separator);
		do
		{
...
void convertData2(FILE* fileIn,FILE* fileOut,char separator,long double* start,long offset)
{
	...
	for(uint64_t ii=0;!feof(fileIn);ii++)
	{
		...
		offset2=ftell(fileIn);
		//Jump to angle offset
		fseek(fileIn, offset, SEEK_SET);
		do
		{
			//Just copy values from input (to avoid unnecessary conversion losses)
		...
		//Backup current offset
		offset=ftell(fileIn);
		//Jump to counts offset
		fseek(fileIn, offset2, SEEK_SET);
		
		//Print separator
		fprintf(fileOut,"%c",separator);
		//Print counts
		do
		{
			//Just copy values from input (to avoid unnecessary conversion losses)
